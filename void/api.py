...
function modelStatus(status){return status&&status.model&&typeof status.model==='object'?status.model:status||{}}
function updateTokens(status){const m=modelStatus(status);const u=m.total_usage||{};const used=Number(u.total_tokens||0);const context=Number(m.context_available||m.context_window||0);$('#tokens').textContent='tokens: '+fmt(used)+' used · context: '+(context?fmt(context)+' available':'—')}
async function loadModels(){try{const r=await fetch('/models',{cache:'no-store'});const j=await r.json();if(!r.ok)throw Error(j.message||j.error||'model discovery failed');$('#model').innerHTML='<option value="">Auto model</option>';for(const m of j.models||[]){const o=document.createElement('option');o.value=m.id;o.textContent=m.id;$('#model').appendChild(o)}const s=await (await fetch('/status',{cache:'no-store'})).json();const m=modelStatus(s);const count=Array.isArray(m.models)?m.models.length:Number(j.count||0);$('#state').textContent=count+' models';updateTokens(s)}catch(e){$('#state').textContent='offline';$('#tokens').textContent='tokens: —'}}
...
$('#status').onclick=async()=>{try{const s=await (await fetch('/status')).json();updateTokens(s);add('VOID STATUS',JSON.stringify(s,null,2))}catch(x){add('VOID ERROR',x.message,'err')}};
...
class VoidAPI:
    ...
    def models(self) -> Dict[str, Any]:
        ...
        if not models:
            try:
                self.core.model.refresh_models(force=True)
                status = self.core.model.get_status()
                models = status.get("models", [])
            except Exception:
                pass
